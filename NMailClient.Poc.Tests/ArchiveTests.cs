...
public class ArchivePathTests
{
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"poc-{Guid.NewGuid():N}.txt");
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), $"poc-{Guid.NewGuid():N}");
...
public class ArchiveBrowseTests : IDisposable
{
    private readonly string _root =
        Path.Combine(Path.GetTempPath(), $"poc-archive-{Guid.NewGuid():N}");
    ...
    {
        var outside = Path.Combine(Path.GetTempPath(), $"poc-outside-{Guid.NewGuid():N}.txt");
