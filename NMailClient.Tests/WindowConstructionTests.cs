...
public class WindowConstructionTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"nmc-wintest-{Guid.NewGuid():N}");
        ...
        var thread = new Thread(() =>
        {
            try
            {
                ...
                var withKey = new NMailClient.Services.Pgp.PgpService(
    ...
    private static ComposeWindow NewComposeWindow(
        Account account, string? keyringDir, NMailClient.Services.Pgp.PgpService? pgp = null)
    {
        var smtp = new SmtpService(account);
        var imap = new ImapService(account);

        return new ComposeWindow(
            account, smtp, imap, NMailClient.ViewModels.ComposeRequest.Blank(),
            new SettingsStore(),
            new OutboxService(_ => smtp, _ => imap),
            new CardDavService(() => ("", "", "")),
            pgp ?? new NMailClient.Services.Pgp.PgpService(keyringDir),
            new NMailClient.Services.Transport.RecipientSecurity());
    }
    ...
    private static void EnsureApplicationWithResources()
    {
        ...
        foreach (var path in new[] { "Themes/Light.xaml", "Themes/Controls.xaml" })
            ...
            {
                Source = new Uri($"pack://application:,,,/NMailClient;component/{path}"),
            });
    }
}
